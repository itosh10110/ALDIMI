...
def eje(titulo: str = "", grid: bool = True) -> dict:
    return dict(title=dict(text=titulo, font=dict(size=11)),
                ...
                showgrid=grid, gridcolor="rgba(128,128,128,0.25)",
...
def vista_usuario(ctx: dict) -> None:
    ...
    with tab_pac:
        ...
        if pac_alto.empty:
            ...
            st.caption(f"{len(pac_alto)} de {len(df_pac)} pacientes. "
                       "Confirmar cada caso con el equipo médico.")

        st.divider()
        st.subheader("Situación general")
        for nivel, etiqueta in [("Alto", "Prioridad alta"), ("Medio", "Prioridad media"),
                                ("Bajo", "Prioridad baja")]:
            cnt = int((df_pac["nivel_riesgo"] == nivel).sum())
            pct = cnt / len(df_pac)
...
def formulario_paciente(ctx: dict) -> None:
    ...
    row = {
        ...
        "meses_en_tratamiento": meses_trat, "num_reingresos": reingresos,
        "presencia_infeccion": int(infeccion == "Sí"),
        "frecuencia_hospitalizacion_3m": frec_hosp,
        "perdida_peso_reciente": int(peso == "Sí"), "num_comorbilidades": comorb,
        "etapa_cancer_enc":               ord_maps["etapa_cancer"][etapa],
        "estado_fisico_enc":              ord_maps["estado_fisico"][estado_fis],
        "estado_nutricional_enc":         ord_maps["estado_nutricional"][estado_nut],
        "adherencia_tratamiento_enc":     ord_maps["adherencia_tratamiento"][adherencia],
        "apoyo_familiar_enc":             ord_maps["apoyo_familiar"][apoyo],
        "acceso_medicamentos_enc":        ord_maps["acceso_medicamentos"][acceso],
        "estado_emocional_paciente_enc":  ord_maps["estado_emocional_paciente"][emocional],
        "grado_instruccion_cuidador_enc": ord_maps["grado_instruccion_cuidador"][instruccion],
    }
    X_new = pd.DataFrame(0.0, index=[0], columns=m_pac["feat_cols"])
    for k, v in row.items():
        if k in X_new.columns:
            X_new.loc[0, k] = v
    for col, val in [("sexo", sexo), ("diagnostico", diagnostico),
                     ("tipo_tratamiento", tratamiento), ("motivo_ingreso", motivo_ing),
                     ("motivo_reingreso", motivo_re), ("lugar_procedencia", lugar)]:
        if f"{col}_{val}" in X_new.columns:
            X_new.loc[0, f"{col}_{val}"] = 1.0

    X_sc  = m_pac["scaler"].transform(X_new.values)
    nivel = m_pac["clases"][m_pac["modelo"].predict(X_sc)[0]]
    ...
    if nivel == "Alto":
        ...
        st.warning("**PRIORIDAD MEDIA** — Seguimiento semanal: revisar alimentación "
                   "y adherencia al tratamiento.")
    else:
        st.success("**PRIORIDAD BAJA** — Continuar con los controles habituales "
                   "según cronograma.")

    conf = {c: f"{probs[i]*100:.0f}%" for i, c in enumerate(m_pac["clases"])}
    st.caption(f"Confianza del sistema — Alta: {conf.get('Alto', '–')} · "
               f"Media: {conf.get('Medio', '–')} · Baja: {conf.get('Bajo', '–')}. "
...
def vista_desarrollador(ctx: dict) -> None:
    ...
    with tab_mod:
        ...
        with col_p:
            with st.container(border=True):
                st.subheader("Pacientes — clasificación multiclase")
                met_pac = m_pac.get("metricas", {})
                if met_pac:
                    df_mp = pd.DataFrame(met_pac).T
                    df_mp.columns = ["F1-Macro", "AUC-Macro", "Accuracy"][:len(df_mp.columns)]
                    st.dataframe(df_mp.round(4), use_container_width=True)
                    fig = go.Figure()
                    for col in df_mp.columns:
                        fig.add_trace(go.Bar(x=df_mp.index, y=df_mp[col], name=col))
                    fig.update_layout(**layout_grafico(260, leyenda=True), barmode="group",
                                      yaxis=eje("Score") | {"range": [0, 1.05]}, xaxis=eje())
                    st.plotly_chart(fig, use_container_width=True)
                st.caption(f"En producción: {m_pac.get('mejor_nombre', '–')} · "
                           f"{len(m_pac.get('feat_cols', []))} features · "
                           "desbalanceo corregido en entrenamiento.")
    ...
    # ── FEATURES ──────────────────────────────────────────────────────────────
    with tab_feat:
        col_fi, col_fp = st.columns(2, gap="large")
...
def main() -> None:
    ...
    with st.sidebar:
        ...
        alta     = int((df_pac["nivel_riesgo"] == "Alto").sum())
        st.markdown(
