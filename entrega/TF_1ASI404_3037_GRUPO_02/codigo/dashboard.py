...
def cargar_datos():
    ...
    if os.path.exists(DB_COMUN):
        ...
        try:
            ...
            return df_inv, df_orig, df_pac, 'BD común (aldimi_core.db)'
        finally:
            con.close()
    ...
    return df_inv, df_orig, df_pac, 'Archivos CSV locales'
...
try:
    df_inv, df_orig, df_pac, fuente_datos = cargar_datos()
...
# ── PLAN DE REPOSICIÓN (predicción sobre la última semana de cada artículo) ──
@st.cache_data
def plan_reposicion():
    ...
    plan = pd.DataFrame({
        ...
        'Categoría':         ult['categoria'].values,
...
with st.sidebar:
    ...
    <div style='font-size:0.82rem;line-height:2'>
      ...
      🔴 <b>{len(agotados)}</b> artículos agotados<br>
...
if pagina == "🏠  Inicio":
    ...
    with k1:
        st.markdown(f"""<div class="kpi danger">
          ...
          <p class="kpi-sub">Artículos sin stock en el almacén</p>
        </div>""", unsafe_allow_html=True)
    with k2:
    ...
    with col_a:
        ...
        if len(urgentes) == 0:
            st.markdown(f"""<div class="r-ok"><p class="r-acc">
              ✅ Ningún artículo en riesgo de agotarse esta semana.</p></div>""",
              unsafe_allow_html=True)
        else:
            ...
            for _, r in urgentes.head(6).iterrows():
                detalle = ('sin stock' if r['Acción recomendada'].startswith('🔴')
                           else f"quedan {r['Stock actual']:g} uds")
                st.markdown(f"""<div class="accion">
                  <b>{r['Código']}</b> · {r['Categoría']} — {detalle},
                  se consumen ~{r['Consumo semanal']:g}/semana
                  <span style="float:right;color:{ROJO};font-weight:700">{r['Riesgo 7 días']}%</span>
                </div>""", unsafe_allow_html=True)
            if len(urgentes) > 6:
                st.caption(f"… y {len(urgentes)-6} artículos más. "
    ...
    with col_c:
        ...
        if len(riesgo_cat):
            ...
            fig_rc.update_layout(**CL(300, dict(l=4, r=10, t=28, b=4), show_legend=True),
                                 barmode='stack',
                                 xaxis=ax(True, 'Artículos en riesgo'),
...
# ══════════════════════════════════════════════════════════════════════════════
    ...
    c_filtro, c_desc = st.columns([3, 1], gap="medium")
    with c_filtro:
        filtro = st.radio("Mostrar", ["Solo con acción pendiente", "Todos los artículos"],
                          horizontal=True, label_visibility="collapsed")
    plan_vista = plan if filtro == "Todos los artículos" else \
    ...
    if len(plan_vista) == 0:
        ...
        st.caption(f"🔴 {len(agotados)} agotados · 🟠 {len(urgentes) - len(agotados)} se agotan esta semana · "
                   ...
                   f"El riesgo es la probabilidad de quedarse sin stock estimada por el sistema.")

    st.markdown("<div style='height:10px'></div>", unsafe_allow_html=True)

    # ── Consulta individual ───────────────────────────────────────────────────
    st.markdown('<p class="st">🔍 ¿Alcanzará el stock? — consulta un artículo</p>', unsafe_allow_html=True)
    ...
    with c1:
        cat_sel      = st.selectbox("Tipo de artículo", m_inv['categorias'])
        stock_inicio = st.number_input("Stock disponible hoy (unidades)", -50.0, 500.0, 8.0, 1.0)
        ingresos     = st.number_input("Donaciones/compras que llegan esta semana", 0.0, 200.0, 0.0, 1.0)
    with c2:
        salidas     = st.number_input("Consumo previsto esta semana", 0.0, 100.0, 3.0, 0.5)
        rolling_avg = st.number_input("Consumo promedio de las últimas 3 semanas", 0.0, 50.0, 2.5, 0.5)
    ...
    with col_ev:
        ...
        cat_sel2 = st.selectbox("Tipo de artículo", sorted(df_inv['categoria'].unique()), key='cat2')
    ...
    with col_top:
        st.markdown('<p class="st">⚡ Artículos que más rápido se consumen</p>', unsafe_allow_html=True)
        ...
        st.caption("Estos artículos conviene pedirlos siempre en las campañas de donación.")
